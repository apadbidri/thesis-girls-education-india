...
# detach("package:MASS", unload = TRUE) # use when needed
...
# Define predictors string
predictors_hasbro <- paste(
  "~ has_brother + wi + sv + hhsize + mother_edu + father_edu +",
  "chldeth + typesite + birth_order + n_siblings +",
  "birth_order * has_brother + wi * has_brother"
)

# Run the regression
mod_height <- lm(as.formula(paste("chheght", predictors_hasbro)), data = girls_df_cleaned)

# Create APA-style table (all covariates shown)
height_table <- msummary(
  mod_height,
  output    = "flextable",
  fmt       = 2,                                   # round to 2 dp
  stars     = c('*' = .10, '**' = .05, '***' = .01),
  gof_map   = tribble(                             # nice GOF labels
    ~raw,            ~clean,          ~fmt, ~omit,
    "r.squared",     "R²",             2,    FALSE,
    "adj.r.squared", "Adjusted R²",    2,    FALSE,
    "nobs",          "N",              0,    FALSE
  )
) |>
  set_caption("OLS Regression Predicting Child Height") |>
  add_footer_lines("Note. Unstandardized coefficients with standard errors in parentheses. * p < .10, ** p < .05, *** p < .01.") |>
  autofit()

# Save to Word
doc <- read_docx() |>
  body_add_flextable(height_table)

print(doc, target = "chheght_regression_APA.docx")
```
...
# -----------------------
# Predictor sets (SISTERS)
# -----------------------
predictors_hassis <- "~ has_sister + wi + sv + hhsize + mother_edu + father_edu +
                      chldeth + typesite + birth_order + n_siblings +
                      birth_order * has_sister + wi * has_sister"

predictors_sis <- "~ sisters + wi + sv + hhsize + mother_edu + father_edu +
                   chldeth + typesite + birth_order + n_siblings +
                   birth_order * sisters + wi * sisters"

# -----------------------
# Outcome-specific data frames (SISTERS)
# -----------------------
outcome_dfs_sis <- list(
  engl_perco  = sis_eng_model_df,
  maths_perco = sis_math_model_df,
  lang_perco  = sis_lang_model_df,
  HGHQULR4    = sis_HGHQULR4_model_df
...
# -----------------------
# Helper: run both specs for a given outcome & subsample (SISTERS)
# -----------------------
run_two_specs_sis <- function(df, outcome, predictors1, predictors2) {
  m1 <- lm(as.formula(paste(outcome, predictors1)), data = df)
  m2 <- lm(as.formula(paste(outcome, predictors2)), data = df %>% filter(sisters < 3))
  list(m1, m2)
}

# -----------------------
# Function to make table for ONE outcome (SISTERS)
# `coef_keep` = TRUE for main text (filtered vars), FALSE for appendix (all vars)
# -----------------------
make_outcome_table_sis <- function(outcome, outcome_label, coef_keep = TRUE, portrait = TRUE) {

  # Full sample
  full_mods  <- run_two_specs_sis(outcome_dfs_sis[[outcome]], outcome, predictors_hassis, predictors_sis)

  # First-born
  first_mods <- run_two_specs_sis(sis_firstborn_dfs[[outcome]], outcome, predictors_hassis, predictors_sis)

  # Later-born
  later_mods <- run_two_specs_sis(sis_laterborn_dfs[[outcome]], outcome, predictors_hassis, predictors_sis)

  # Combine into a single named list
  all_mods <- list(
    "Full sample: Has sister"     = full_mods[[1]],
    "Full sample: Sisters < 3"    = full_mods[[2]],
    "First-born: Has sister"      = first_mods[[1]],
    "First-born: Sisters < 3"     = first_mods[[2]],
    "Later-born: Has sister"      = later_mods[[1]],
    "Later-born: Sisters < 3"     = later_mods[[2]]
  )

  # Variable labels (SISTERS)
  var_labels <- c(
    ...
    "birth_order:sisters"     = "Birth order × Number of sisters",
    "wi:sisters"              = "Wealth index × Number of sisters"
  )

  # What to keep in main text
  keep_vars <- if (coef_keep) names(var_labels) else NULL
  ...
  ft <- msummary(
    ...
    fmt       = 2,                                   # round to 2 dp
    stars     = c('*' = .10, '**' = .05, '***' = .01),
    gof_omit  = 'IC|Log|Adj|F',                      # keep R2, Adj R2 if you prefer: use gof_map instead
    coef_map  = var_labels,
    coef_omit = if (!coef_keep) NULL else paste0("^(?!", paste(keep_vars, collapse="|"), ").*$")
  ) |>
    set_caption(paste0("OLS Regression Results — ", outcome_label))
...
# MAIN TEXT TABLES (portrait-friendly)
# -----------------------
tab_eng_main_sis  <- make_outcome_table_sis("engl_perco",  "English test score", TRUE, TRUE)
tab_math_main_sis <- make_outcome_table_sis("maths_perco", "Mathematics test score", TRUE, TRUE)
tab_lang_main_sis <- make_outcome_table_sis("lang_perco",  "Language test score", TRUE, TRUE)
tab_edu_main_sis  <- make_outcome_table_sis("HGHQULR4",    "Highest educational attainment", TRUE, TRUE)

# -----------------------
# APPENDIX TABLES (landscape, full)
# -----------------------
tab_eng_app_sis  <- make_outcome_table_sis("engl_perco",  "English test score", FALSE, FALSE)
tab_math_app_sis <- make_outcome_table_sis("maths_perco", "Mathematics test score", FALSE, FALSE)
tab_lang_app_sis <- make_outcome_table_sis("lang_perco",  "Language test score", FALSE, FALSE)
tab_edu_app_sis  <- make_outcome_table_sis("HGHQULR4",    "Highest educational attainment", FALSE, FALSE)

# -----------------------
# EXPORT TO WORD (SISTERS)
# -----------------------
doc_sis <- read_docx()

# Main text (Portrait)
doc_sis <- body_add_par(doc_sis, "Main Text Tables — Sisters (Portrait)", style = "heading 1")
doc_sis <- body_add_flextable(doc_sis, tab_eng_main_sis)
doc_sis <- body_add_par(doc_sis, "")
doc_sis <- body_add_flextable(doc_sis, tab_math_main_sis)
doc_sis <- body_add_par(doc_sis, "")
doc_sis <- body_add_flextable(doc_sis, tab_lang_main_sis)
doc_sis <- body_add_par(doc_sis, "")
doc_sis <- body_add_flextable(doc_sis, tab_edu_main_sis)
...
doc_sis <- body_add(doc_sis, block_section(landscape_props))


# Appendix (Landscape)
doc_sis <- body_add_par(doc_sis, "Appendix Tables — Sisters (Full Results, Landscape)", style = "heading 1")
doc_sis <- body_add_par(doc_sis, "")
doc_sis <- body_add_flextable(doc_sis, tab_eng_app_sis)
doc_sis <- body_add_par(doc_sis, "")
doc_sis <- body_add_flextable(doc_sis, tab_math_app_sis)
doc_sis <- body_add_par(doc_sis, "")
doc_sis <- body_add_flextable(doc_sis, tab_lang_app_sis)
doc_sis <- body_add_par(doc_sis, "")
doc_sis <- body_add_flextable(doc_sis, tab_edu_app_sis)

# (Optional) Switch back to Portrait if you plan to add more content after
...
doc_sis <- body_add(doc_sis, block_section(portrait_props))

print(doc_sis, target = "OLS_results_SISTERS_main_portrait_appendix_landscape.docx")
